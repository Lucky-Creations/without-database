...
function renderCatalogue(list) {
  ...
  list.forEach(item => {
    ...
    const webpUrl = `https://lucky-creations.github.io/images/tricolour/${enc(base)}.webp`;
    const pngUrl  = `https://lucky-creations.github.io/images/tricolour/${enc(file)}`;
...
fetch('../json-files/tricolour.json')
  ...
  .then(products => {
    ...
    document.querySelector('#filters select').addEventListener('change', e => {
      ...
      } else if (filter === "keychains") {
        filtered = products.filter(p => p.category === "keychains");
    ...
    function applyFilterAndSort() {
      ...
      if (currentFilter === "mini") {
        list = list.filter(p => p.category === "mini");
      } else if (currentFilter === "scrunchies") {
        list = list.filter(p => p.category === "scrunchies");
      } else if (currentFilter === "keychains") {
        list = list.filter(p => p.category === "keychains");
...
.catch(err => {
  ...
  console.error('Error loading everything tricolour:', err);
});