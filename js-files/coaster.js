...
function renderCatalogue(list) {
  ...
  list.forEach(item => {
    ...
    const webpUrl = `https://lucky-creations.github.io/images/coaster-sets/${enc(base)}.webp`;
    const pngUrl  = `https://lucky-creations.github.io/images/coaster-sets/${enc(file)}`;
  ...
  .then(products => {
    ...
    function applyFilterAndSort() {
      ...
      if (currentFilter === "misc") {
        list = list.filter(p => p.category === "misc");
      } else if (currentFilter === "round") {
        list = list.filter(p => p.category === "round");
      }
...
.catch(err => {
  ...
  console.error('Error loading coaster sets:', err);
});