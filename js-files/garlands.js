// Rendering curtain gaga over garlands
function renderCatalogue(list) {
  ...
  list.forEach(item => {
    ...
    const webpUrl = `https://lucky-creations.github.io/images/garlands/${enc(base)}.webp`;
    const pngUrl  = `https://lucky-creations.github.io/images/garlands/${enc(file)}`;
    ...
    card.addEventListener('click', (e) => {
      ...
      const modalGarlandContent = document.querySelector('.modal-garland-content');
      ...
      modalGarlandContent.querySelectorAll('picture').forEach(p => p.remove());
      ...
      if (Array.isArray(item.sequence)) {
        item.sequence.forEach(file => {
          ...
          // Appends directly inside modal-garland-content, after the hero image
          modalGarlandContent.appendChild(picture);
        });
      }
...
fetch('../json-files/garlands.json')
  .then(res => res.json())
  .then(products => {
...
.catch(err => {
  ...
  console.error('Error loading curtain garlands:', err);
});