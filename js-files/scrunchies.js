...
function renderCatalogue(list) {
  ...
  list.forEach(item => {
    ...
    const webpUrl = `https://lucky-creations.github.io/images/scrunchies/${enc(base)}.webp`;
    const pngUrl  = `https://lucky-creations.github.io/images/scrunchies/${enc(file)}`;
...
fetch('../json-files/scrunchies.json')
  .then(res => res.json())
  .then(products => {
    ...
    function applyFilterAndSort() {
      ...
      if (currentFilter === "mono") {
        list = list.filter(p => p.category === "mono");
      } else if (currentFilter === "multi") {
        list = list.filter(p => p.category === "multi");
      }
...
.catch(err => {
  ...
  console.error('Error loading scrunchies:', err);
});