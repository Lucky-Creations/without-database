...
function renderCatalogue(list) {
  ...
  list.forEach(item => {
    ...
    const webpUrl = `https://lucky-creations.github.io/images/centerpieces/${enc(base)}.webp`;
    const pngUrl  = `https://lucky-creations.github.io/images/centerpieces/${enc(file)}`;
    ...
    card.className = 'centerpiece-card';
...
fetch('../json-files/centerpieces.json')
  .then(res => res.json())
  .then(products => {
    ...
    document.querySelector('#filters select').addEventListener('change', e => {
      ...
      if (filter === "dual") {
        ...
        filtered = products.filter(p => p.category === "thematic");
      } else if (filter === "mono") {
    ...
    function applyFilterAndSort() {
      ...
      } else if (currentFilter === "thematic") {
        list = list.filter(p => Array.isArray(p.category) ? p.category.includes("thematic") : p.category === "thematic" );
...
.catch(err => {
  ...
  console.error('Error loading centerpieces:', err);
});