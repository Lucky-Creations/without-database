...
function renderCatalogue(list) {
  ...
  list.forEach(item => {
    ...
    const webpUrl = `https://lucky-creations.github.io/images/mini-bags/${enc(base)}.webp`;
    const pngUrl  = `https://lucky-creations.github.io/images/mini-bags/${enc(file)}`;
...
fetch('../json-files/mini.json')
  ...
  .then(products => {
    ...
    document.querySelector('#filters select').addEventListener('change', e => {
      ...
      } else if (filter === "pouch") {
        filtered = products.filter(p => p.category === "pouch");
      }
    ...
    function applyFilterAndSort() {
      ...
      } else if (currentFilter === "pouch") {
        list = list.filter(p => Array.isArray(p.category) ? p.category.includes("pouch") : p.category === "pouch" );
      }
...
.catch(err => {
  ...
  console.error('Error loading mini bags:', err);
});