...
function renderCatalogue(list) {
  ...
  list.forEach(item => {
    ...
    const webpUrl = `https://lucky-creations.github.io/images/keychains/${enc(base)}.webp`;
    const pngUrl  = `https://lucky-creations.github.io/images/keychains/${enc(file)}`;
...
fetch('../json-files/keychains.json')
  .then(res => res.json())
  .then(products => {
    ...
    document.querySelector('#filters select').addEventListener('change', e => {
      ...
      if (filter === "animated") {
        filtered = products.filter(p => p.category === "animated");
      } else if (filter === "floral") {
        filtered = products.filter(p => p.category === "floral");
      } else if (filter === "puffed") {
        filtered = products.filter(p => p.category === "puffed");
      }
    ...
    function applyFilterAndSort() {
      ...
      if (currentFilter === "animated") {
        list = list.filter(p => p.category === "animated");
      } else if (currentFilter === "floral") {
        list = list.filter(p => p.category === "floral");
      } else if (currentFilter === "puffed") {
        list = list.filter(p => p.category === "puffed");
      }
