...
function renderCatalogue(list) {
  ...
  list.forEach(item => {
    ...
    const webpUrl = `https://lucky-creations.github.io/images/rakhis/${enc(base)}.webp`;
    const pngUrl  = `https://lucky-creations.github.io/images/rakhis/${enc(file)}`;
    ...
    card.addEventListener('click', (e) => {
      ...
      const modalRakhiContent = document.querySelector('.modal-rakhi-content');
      ...
      modalRakhiContent.querySelectorAll('picture').forEach(p => p.remove());
      ...
      if (Array.isArray(item.sequence)) {
        item.sequence.forEach(file => {
          ...
          // Appends directly inside modal-rakhi-content, after the hero image
          modalRakhiContent.appendChild(picture);
        });
      }
...
fetch('../json-files/rakhis.json')
  ...
  .then(products => {
    ...
    document.querySelector('#filters select').addEventListener('change', e => {
      ...
      if (filter === "happy") {
        filtered = products.filter(p => p.category === "happy");
      } else if (filter === "pearl") {
        filtered = products.filter(p => p.category === "pearl");
      } else if (filter === "twine") {
        filtered = products.filter(p => p.category === "twine");
      }
    ...
    function applyFilterAndSort() {
      ...
      if (currentFilter === "happy") {
        list = list.filter(p => Array.isArray(p.category) ? p.category.includes("dual") : p.category === "happy" );
      } else if (currentFilter === "pearl") {
        list = list.filter(p => Array.isArray(p.category) ? p.category.includes("multi") : p.category === "pearl" );
      } else if (currentFilter === "twine") {
        list = list.filter(p => Array.isArray(p.category) ? p.category.includes("thematic") : p.category === "twine" );
      }
...
.catch(err => {
  ...
  console.error('Error loading rakhis:', err);
});